...
export class FrmTecnico {

  titulo!: string;
  
    private srvTecnico = inject(TecnicoService); 
    ...
    constructor() {
      this.myForm = this.builder.group({
        id: [0],
        idTecnico: ['', [Validators.required, Validators.minLength(9), Validators.maxLength(15),
    ...
    if (this.myForm.value.id === 0) {
      this.srvTecnico.guardar(this.myForm.value)?.subscribe({
        ...
        error: (err) => {
          this.dialog.open(DialogoGeneral, {
            data: {
              texto: 'Error al insertar técnico. Inténtelo de nuevo.',
              icono: 'error',
              textoAceptar: 'Aceptar'
            }
          });
      ...
      this.srvTecnico.guardar(this.myForm.value, this.myForm.value.id)
        ?.subscribe({
          complete: () => {
            this.dialog.open(DialogoGeneral, {
              data: {
          ...
          error: (err) => {
            this.dialog.open(DialogoGeneral, {
              data: {
                texto: 'Error al modificar técnico. Inténtelo de nuevo.',
                icono: 'error',
                textoAceptar: 'Aceptar'
              }
            });
