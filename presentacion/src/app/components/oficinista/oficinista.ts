...
export class Oficinista implements AfterViewInit {
  ...
  columnas: string[] = ['idOficinista', 'nombre', 'apellido1', 'apellido2', 'celular', 'correo',
    'botonera'];

  dataSource = signal(new MatTableDataSource<TipoOficinista>());
  filtro: any;

  mostraDialogo(titulo: string, datos?: TipoOficinista) {
  ...
  onNuevo() {
    this.mostraDialogo('Nuevo Oficinista');
  }
  ...
  onEditar(id: number) {
    this.oficinistaSrv.buscar(id)
      .subscribe({
        next: (data) => {
          this.mostraDialogo('Editar Oficinista', data);
        }
      })
  }
      ...
      .subscribe({
        next: (data) => {
          ...
          dialogRef.afterClosed().subscribe(result => {
            if (result === true) {
              this.usuarioSrv.resetearPassw(data.idOficinista)
                .subscribe(() => {
  ...
  onEliminar(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this.oficinistaSrv.eliminar(id)
          .subscribe({
            next: (res: any) => {
              this.resetearFiltro();
              this.dialogo.open(DialogoGeneral, {
            ...
            error: (err) => {
              this.dialogo.open(DialogoGeneral, {
                data: {
                  texto: 'Error al eliminar oficinista. Inténtelo de nuevo.',
                  icono: 'error',
                  textoAceptar: 'Aceptar'
                }
              });
  ...
  onImprimir() {
    const encabezado = [
      'IdOficinista', 'Nombre', 'Teléfono', 'Celular', 'Correo'
    ];
    this.oficinistaSrv.filtrar(this.filtro)
      .subscribe({
        next: (data) => {
